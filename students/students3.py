...
class NotArgError(Exception): #自定义异常类
    def __init__(self, message): #构造参数
...
class StudentInfo(object):
    ...
    def add(self, **student): #对add函数的check_user_info进行异常捕获
        try:
    ...
    def update(self, student_id, **kwargs):
        ...
        try:
            self.check_user_info(**kwargs)  #依然是对check_user_info进行升级
        except Exception as e:
            raise e
    ...
    def updates(self, update_students):
        for student in update_students:
            try: #判断传入成员是否合法的异常捕获
                ...
                print(e)
                continue #continue保证下一个进来的同学也顺利更新
            if id_ not in self.students:
                print(f'学号{id_} 不存在')
                continue
            user_info = student[id_]
            try:
                self.check_user_info(**user_info) #对check_user_info添加异常捕获
            except Exception as e:
                print(e)
                continue
    ...
    def check_user_info(self, **kwargs): #着重调整的部分
        assert len(kwargs) == 4, '参数必须是4个' #判断传入参数是不是四个

        #验证四个key对不对
        if 'name' not in kwargs:
            raise NotArgError('没有发现学生姓名参数') #自定义的异常类“NotArgError”
        ...
        #判断四个value 运用isinstance函数 ，可帮助判断某个值是不是某个数据类型
